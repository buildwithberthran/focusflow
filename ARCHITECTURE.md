...
`src/components/Layout/Sidebar.tsx` renders three things now, shown/hidden purely by CSS media query (no JS breakpoint logic): the desktop vertical `.sidebar`, a mobile `.mobile-topbar` (brand + account), and a mobile `.mobile-tabbar` (fixed to the bottom, icon + tiny label, the same 5 sections). The old approach just squished the desktop sidebar sideways into one cramped row on narrow screens — replaced because a bottom tab bar is the actual native mobile pattern, not a shrunk-down desktop sidebar. `ProgressRing` no longer renders fixed pixel `width`/`height` on its `<svg>` — it uses `width:100%` inside a wrapper capped at `max-width:100%`, so it shrinks to fit narrow screens instead of overflowing. Smaller, targeted fixes (grid columns collapsing, `.recover-item` stacking, tables scrolling horizontally) live in the `@media` blocks near the bottom of `src/index.css`.

## Pop-out window styles

Migration `0008` adds `popup_style: 'ring' | 'minimal' | 'bar'` to `user_settings`, picked from Settings → Pop-out window. In `useFocusTimer.ts`, `getWidgetTemplate()` picks between three self-contained CSS+HTML template pairs (`widgetCSSRing`/`widgetBodyHTMLRing`, and the `Minimal`/`Bar` equivalents) plus a matching window size for each — Ring keeps the original circular progress ring, Minimal is just the timer digits, Bar swaps the ring for a linear progress bar. `applyWidgetState()` stays a single function for all three: it only touches elements that actually exist in the current template (a `pw-ring-fill` circle in Ring, a `pw-bar-fill` div in Bar), so adding a 4th style later just means adding another CSS/HTML pair and, if it introduces a genuinely new element, one more `if (el) {...}` block.
...
- **Target mode: extend or restart a cycle** (migration `0007`) — one log entry per work block, always. `duration_min` never changes (it's the original plan); extensions accumulate in `extension_log` (a plain list of added minutes) so History can show "20m → +15m → +12m = 47m total" without ever hiding the original estimate. Restarting resets progress (countdown, pause time) but *not* `extension_log` — a restart is "give this a clean run," not "erase what this has already cost." `restart_count` increments, shown as a small "Restarted N×" badge. (Earlier version of this feature cleared `extension_log` on restart; changed after realizing that quietly discarded exactly the accountability data the feature exists to capture.) `dbAppendCycleExtension`/`dbRegisterCycleRestart` in `db.ts` do read-then-write so concurrent calls don't clobber each other.
